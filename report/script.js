const lab1Info = {
    "topic": "Тема: СТРУКТУРА HTML-ДОКУМЕНТА. ВИБІР ПРЕДМЕТНОЇ ГАЛУЗІ. СТРУКТУРА HTML-ДОКУМЕНТА. ВИБІР ПРЕДМЕТНОЇ ГАЛУЗІ. HTML-ДОКУМЕНТІ.",
    "goal": "Мета: придбати практичні навички роботи з HTML-документом, таблицями, зображеннями, посиланнями, списками, формами. " +
        "Створити шаблон звітного HTML-документом для відображення результатів роботи всіх лабораторних робіт.",
    "info_2":
        `Вебсайт guidebook по всесвіту «Зоряні війни». На вебсайті доступний вибір по категоріям
    ...
    "info_4": "lab1.png",
    "info_5": "Висновки роботи: виконуючи дану лабораторну роботу ми навчились створювати HTML-документ і використовувати елементи, такі як: " +
        "таблиці, зображення, посилання, списки, форми.",
    "info_6": "Виконали роботу: Кононенко Вадим ІП-05, Комісар Кіріл ІП-05, Рогозний Олексій ІП-05"
}

const lab2Info = {
    "topic": "Тема: КАСКАДНІ ТАБЛИЦІ СТИЛІВ. СЕЛЕКТОРИ .ІДЕНТИФІКАТОРИ. СТИЛЬОВЕ ОФОРМЛЕННЯ ТЕКСТОВИХ ЕЛЕМЕНТІВ В HTML-ДОКУМЕНТАХ.",
    "goal": "Мета: придбати практичні навички роботи з селекторами, ідентифікаторами, списками, різноманітними властивостями кольору і фону, " +
...
document.addEventListener('DOMContentLoaded', setupFirstLabView);

let currentModel = lab1Info;

function setupFirstLabView() {
...
function updateInfo(modelNumber) {
    if (modelNumber === 1) {
        currentModel = lab1Info;
        setupFirstLabView()
    } else if (modelNumber === 2) {
        currentModel = lab2Info;
        showTopicAndGoalInfo()
    }
}

function showTopicAndGoalInfo() {
    let placeOut = document.getElementById('place-out');
    placeOut.textContent = "";

    let topic = document.createElement('p');
    topic.textContent = currentModel.topic;

    let goal = document.createElement('p');
    goal.textContent = currentModel.goal;

    placeOut.appendChild(topic);
    placeOut.appendChild(goal);
}

function showDescribingInfo() {
    let placeOut = document.getElementById('place-out');
    placeOut.textContent = "";

    let p = document.createElement('p');
    p.textContent = currentModel.info_2;
...
function showAppearanceOfLab() {
    ...
    let img = document.createElement('img');
    img.src = "assets/" + currentModel.info_4;
...
function showConclusion() {
    ...
    let p = document.createElement('p');
    p.textContent = currentModel.info_5;
...
function showWorkers() {
    ...
    let p = document.createElement('p');
    p.textContent = currentModel.info_6;
